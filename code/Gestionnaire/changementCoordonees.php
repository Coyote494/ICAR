		...
		function displayRequest(){
			...
			while(($new_content = fgetcsv($new)) !== FALSE){
				if($new_content[0] == 0){
					...
					echo "<a href=".$new_content[6]." target='_blank'>voir le justificatif</a>";
					echo "<input action'#' type='button' value='confirmer la demande'>";
					fclose($old);
				}else{
					echo "<p>Demande de changement de coordonnées</p>";
					$old = fopen("../../database/Allianz"/*.$_SESSION['assurance']*/."/".strtoupper(substr($new_content[6],0,1))."/".$new_content[6]."_".$new_content[7]."/coordonnees.csv", "r");
					$old_content = fgetcsv($old);
					...
					echo "<a href=".$new_content[8]." target='_blank'>voir le justificatif</a>";
					echo "<input action '#' type='button' value='confirmer la demande'>";
					fclose($old);
				}
			}
